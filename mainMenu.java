...
public class mainMenu extends BasicGameState {
    ...
    public void init(GameContainer gc, StateBasedGame sbg) throws SlickException {
        ...
        Rect9=new RoundedRectangle(100,430,622,70,20);
        Rect10=new RoundedRectangle(100,430,622,63,20);
    ...
    public void render(GameContainer gc, StateBasedGame sbg, Graphics g) throws SlickException {
        ...
        font.drawString(280, 430, "Multiplayer", color3);
        if(a)
            sbg.enterState(3);
        if(b)
            sbg.enterState(1);
    ...
    public void update(GameContainer gc, StateBasedGame sbg, int delta) throws SlickException {
        ...
        if(xpos<=722&&xpos>=100&&ypos<=143&&ypos>=79)
            ...
            i=false;
        if(xpos<=720&&xpos>=100&&ypos<=70&&ypos>=5)
            l=true;
        else
            l=false;
        if(xpos<=720&&xpos>=100&&ypos<=70&&ypos>=5&&input.isMousePressed(input.MOUSE_LEFT_BUTTON))
            m=true;
        else
            m=false;
